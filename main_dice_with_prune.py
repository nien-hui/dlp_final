...
def test_prune_noisy(new_adj):
    ...
    for beta in np.arange(0, args.beta_max, args.beta_min): 
        ...
        classifier.fit(features_local, new_adj, labels, idx_train, 
                        train_iters=150, idx_val=idx_val, idx_test=idx_test, 
                        verbose=False, attention=False) 
        classifier.eval() 
...
if __name__ == '__main__': 
    ...
    attention=False 
    acc_noise_clean=test_prune_noisy(adj) 
    acc_noise_attacked=test_prune_noisy(modified_adj) 
    print('---------------')
    print("NoisyPGCN Non Attacked Acc - {}" .format(acc_noise_clean))
    print("NoisyPGCN Attacked Acc - {}" .format(acc_noise_attacked))
    print('---------------')
